...
async function loadClients() {
  ...
  if (error) {
    clientList.innerHTML =
      '<p class="portal-error">Could not load clients.</p>';
    return;
  }
  ...
  if (ids.length) {
    const { data: profiles, error: profilesError } = await sb
      .from('profiles')
      .select('id,full_name,business_name,last_login_at')
      .in('id', ids);

    if (profilesError) {
      console.error('Could not load client profiles:', profilesError);
    }
  ...
  clients = rows.map((r) => ({
    ...r,
    profile: profilesById[r.client_id] || null
  }));

  clientList.innerHTML = clients.length
    ? clients
        .map((c) => {
          const lastLogin = formatLastLogin(
            c.profile?.last_login_at
          );

          return `
            <button class="client-item" data-id="${c.id}">
              <strong>${esc(
                c.profile?.full_name || 'Client'
              )}</strong>

              <small>
                ${esc(c.service_name || 'Client')}
                ·
                ${c.progress || 0}%
              </small>

              <small class="client-last-login">
                ${lastLogin.icon}
                Last login: ${esc(lastLogin.text)}
              </small>
            </button>
          `;
        })
...
function openClient(id) {
  ...
  editHeading.textContent =
    c.profile?.full_name || 'Edit client';
  ...
  editStage.value =
    c.current_stage || 'Information received';

  editAction.value =
    c.next_action || '';

  editActionDetail.value =
...
async function saveClient(e) {
  ...
  const payload = {
    ...
    next_action:
      editAction.value.trim(),

    next_action_detail:
      editActionDetail.value.trim(),
  ...
  show(
    adminMessage,
    error
      ? `Could not save the update: ${error.message}`
      : 'Client updated ✓',
    !error
  );
...
async function sendMessage() {
  ...
  show(
    adminMessage,
    error
      ? `Could not add the message: ${error.message}`
      : 'Message added to client portal ✓',
    !error
  );
